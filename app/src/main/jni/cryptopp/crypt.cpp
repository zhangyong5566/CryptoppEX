#include <cstdlib>
#include <string>
#include "../../../../../../../AndroidSDK/android-ndk-r16b/sources/cxx-stl/gnu-libstdc++/4.9/include/cstring"
...
static void xShiftSubst(xword32 res[MAXBC], xword32 a[MAXBC],
...
static void xMixAdd(xword32 res[MAXBC], xword32 a[MAXBC],
		    xword32 rk[MAXBC], int BC)
{
...
static void xInvMixColumn(xword32 res[MAXBC], xword32 a[MAXBC], int BC)
{
...
int aes_encrypt(char * key, char * in, int len, char * out, int size)
{
	...
	pad_size = aes_getsize(len);
	if (size < pad_size)
		return 0;

	hashstring(key, keyblock);
	
	xrijndaelKeySched(keyblock, 256, 256, &rk);

 memcpy(out, in, len);
	out[pad_size-1] = 32 - len % 32;
	offset = 0;
	while (offset < pad_size) 
	{
		xrijndaelEncrypt((xword32*)(out+offset), &rk);
		offset += 32;
	}
	return pad_size;
}
...
int aes_decrypt(char * key, char * in, int len, char * out, int size)
{
	...
	while (offset < len)
	{
		xrijndaelDecrypt((xword32*)(out+offset), &rk);
		offset += 32;
	}
	ret = len-out[len-1]-32;
	if (ret > size)
		return size;
	else if (ret < 0)
		return 0;
...
int RadomString(char * table, int len, std::string & str)
{
    ...
    str.clear();
    for (int i = 0; i < len; ++i)
        str += table[RangeRandom(0, table_len)];
